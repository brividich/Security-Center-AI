...
export function EventTable({ items, compact = false }: EventTableProps) {
  const [selected, setSelected] = useState<InboxItem>(items[0]);
    ...
    <section className="grid gap-4 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 xl:grid-cols-[1fr_0.9fr]">
      <div>
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="font-bold text-slate-950">Event Inbox</h2>
            <p className="text-sm text-slate-500">Lista normalizzata, già deduplicata e ordinata per priorità.</p>
          </div>
          {!compact ? <SeverityBadge tone="dark">7 open</SeverityBadge> : null}
        </div>
      ...
      <aside className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
        <SeverityBadge tone={toneForSeverity(selected.severity)}>{selected.severity}</SeverityBadge>
        <h3 className="mt-3 text-lg font-bold text-slate-950">{selected.title}</h3>
        <dl className="mt-4 space-y-4 text-sm">
          <div>
            <dt className="font-semibold text-slate-500">Perché è stato creato</dt>
