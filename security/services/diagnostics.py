...
def parser_registry_diagnostics():
    ...
    for name in sorted(registry_names | set(configured)):
        ...
        if config and name not in registry_names:
            flags.append("Configured parser missing from code registry")
        if name in registry_names and not config:
            flags.append("Registry parser missing config")
        if config and not config.enabled and source_activity:
            flags.append("Disabled parser has recent matching source activity")
        rows.append(
            {
                "parser_name": name,
...
def match_source_sample(sender="", subject="", body=""):
    ...
    return {
        ...
        "selected_reason": selected["reason"] if selected else "No configured source matched the sample.",
    }
...
def alert_rule_diagnostics():
    ...
    for rule in SecurityAlertRuleConfig.objects.order_by("-enabled", "severity", "code"):
        ...
        if rule.condition_operator not in VALID_OPERATORS:
            flags.append("Unsupported operator")
        if _invalid_threshold(rule):
            flags.append("Invalid threshold value")
        if rule.severity == Severity.CRITICAL and not rule.auto_create_evidence_container:
            flags.append("Critical severity rule without auto evidence")
        if _is_defender_critical_cve_rule(rule) and not rule.auto_create_ticket:
            flags.append("Defender critical CVE rule without auto ticket")
        row = {
            "rule": rule,
...
def suppression_diagnostics():
    ...
    for rule in rules:
        ...
        if rule.is_active and rule.severity == Severity.CRITICAL and not (rule.source_id or rule.event_type or rule.conditions_json or rule.match_payload):
            flags.append("Broad suppression on Critical alerts")
        if rule.is_active and not rule.expires_at:
            flags.append("Suppression without expiration")
        if rule.hit_count >= 100:
            flags.append("Very high hit count")
        if not rule.owner:
            flags.append("Missing owner")
        if not rule.reason:
            flags.append("Missing reason")
...
def backup_diagnostics():
    ...
    for config in BackupExpectedJobConfig.objects.order_by("-enabled", "-critical_asset", "job_name"):
        ...
        if config.critical_asset and not config.enabled:
            flags.append("Expected job disabled for critical asset")
...
def notification_diagnostics():
    ...
    for channel in SecurityNotificationChannel.objects.order_by("-enabled", "channel_type", "name"):
        ...
        if channel.enabled and channel.channel_type == "teams_webhook" and not channel.webhook_url_secret_ref:
            flags.append("Enabled Teams webhook missing secret")
        if channel.enabled and channel.channel_type == "email" and not channel.recipients.strip():
            flags.append("Enabled email channel missing recipients")
        rows.append(
            {
                "channel": channel,
...
def _database_check():
    try:
        with connection.cursor() as cursor:
            ...
            cursor.fetchone()
        return _check("database", "Database", "ok", "Database connection is available.")
    except Exception as exc:  # pragma: no cover - defensive diagnostics
        return _check("database", "Database", "error", "Database connection failed.", {"error": str(exc)}, "Verify database connectivity and credentials.")
...
def _migrations_check():
    try:
        ...
        return _check("migrations", "Migrations", "warning", "Pending or invalid migrations detected.", {"exit_code": exc.code}, "Run python manage.py migrate after reviewing migration state.")
    except Exception as exc:
        return _check("migrations", "Migrations", "warning", "Pending or invalid migrations detected.", {"error": str(exc)}, "Run python manage.py migrate after reviewing migration state.")
    return _check("migrations", "Migrations", "ok", "No pending migrations detected.")
...
def _config_seeded_check():
    ...
    return _check(
        ...
        "Security config seeded",
        "warning" if missing else "ok",
        "Configuration seed data is present." if not missing else f"Missing configuration sections: {', '.join(missing)}.",
        counts,
        "Run python manage.py seed_security_center_config." if missing else "",
    )


def _enabled_sources_check():
    count = SecuritySourceConfig.objects.filter(enabled=True).count()
    return _check("enabled_sources", "Enabled sources", "ok" if count else "warning", f"{count} enabled source(s) configured.", {"enabled_count": count}, "Enable at least one source.")


def _enabled_parsers_check():
    count = SecurityParserConfig.objects.filter(enabled=True).count()
    return _check("enabled_parsers", "Enabled parsers", "ok" if count else "warning", f"{count} enabled parser(s) configured.", {"enabled_count": count}, "Enable at least one parser.")
...
def _defender_source_check():
    ...
    return _check("defender_source_for_parser", "Microsoft Defender source", status, "Microsoft Defender source coverage checked.", {"defender_parser_exists": defender_parser, "defender_source_exists": defender_source}, "Create a Microsoft Defender source config." if status == "warning" else "")
...
def _watchguard_sources_check():
    ...
    return _check("watchguard_sources_for_parsers", "WatchGuard sources", status, "WatchGuard source coverage checked.", {"watchguard_parser_exists": watchguard_parser, "watchguard_source_exists": watchguard_source}, "Create WatchGuard source configs." if status == "warning" else "")
...
def _critical_alert_rules_check():
    ...
    return _check("critical_alert_rules", "Critical alert rules", "ok" if count else "warning", f"{count} enabled critical alert rule(s) configured.", {"enabled_critical_rules": count}, "Enable critical alert rules.")


def _ticket_config_check():
    exists = SecurityTicketConfig.objects.exists()
    return _check("ticket_auto_creation_config", "Ticket auto-creation config", "ok" if exists else "warning", "Ticket configuration exists." if exists else "Ticket configuration is missing.", {}, "Create ticketing configuration.")


def _dashboard_notification_check():
    exists = SecurityNotificationChannel.objects.filter(channel_type="dashboard").exists()
    return _check("dashboard_notification_channel", "Dashboard notification channel", "ok" if exists else "warning", "Dashboard notification channel exists." if exists else "Dashboard-only notification channel is missing.", {}, "Create a dashboard notification channel.")
...
def _notification_configuration_check():
    ...
    for channel in SecurityNotificationChannel.objects.filter(enabled=True):
        ...
        if channel.channel_type == "email" and not channel.recipients.strip():
            bad.append(channel.name)
    return _check("notification_required_configuration", "Notification channel configuration", "warning" if bad else "ok", "Enabled notification channels have required configuration." if not bad else f"{len(bad)} enabled channel(s) are missing required configuration.", {"channels": bad}, "Fill required channel destinations/secrets.")
...
def _expired_suppression_check():
    ...
    return _check("expired_active_suppressions", "Expired active suppressions", "warning" if count else "ok", f"{count} expired suppression rule(s) still marked active.", {"count": count}, "Deactivate or renew expired suppression rules." if count else "")
...
def _backup_expectations_check():
    ...
    return _check("backup_expected_jobs", "Backup expectations", "warning" if enabled and count == 0 else "ok", f"{count} enabled expected backup job(s) configured.", {"backup_monitoring_enabled": enabled, "enabled_expected_jobs": count}, "Add expected backup jobs or disable backup monitoring." if enabled and count == 0 else "")
...
def _parser_config_reference_check():
    ...
    return _check("parser_configs_reference_existing_names", "Parser config references", "warning" if missing else "ok", "Source parser references are valid." if not missing else f"{len(missing)} source config(s) reference missing parsers.", {"missing": [{"source": source, "parser_name": parser} for source, parser in missing]}, "Update source parser names or register the missing parser.")
...
def _missing_parser_config_check():
    ...
    return _check("registry_parsers_have_config", "Parser registry configuration", "warning" if missing else "ok", "All registry parsers have config rows." if not missing else f"{len(missing)} registry parser(s) are missing config.", {"missing": missing}, "Run the seed command or create parser config rows.")
...
def _alert_rule_operator_check():
    ...
    return _check("alert_rule_valid_operators", "Alert rule operators", "warning" if invalid else "ok", "Alert rule operators are valid." if not invalid else f"{len(invalid)} alert rule(s) use unsupported operators.", {"invalid": [{"code": code, "operator": operator} for code, operator in invalid]}, "Replace unsupported operators with supported choices.")
...
def _setting_type_check():
    ...
    return _check("setting_value_types", "Setting value types", "warning" if invalid else "ok", "Setting values match declared types." if not invalid else f"{len(invalid)} setting(s) do not match declared types.", {"invalid": invalid}, "Correct setting values or declared value types.")


def _secret_rendering_check():
    secret_count = SecurityCenterSetting.objects.filter(is_secret=True).exclude(value__in=["", None]).count()
    return _check("secret_values_redacted", "Secret settings redaction", "ok", f"{secret_count} secret setting(s) will be rendered as masked values only.", {"secret_settings": secret_count}, "")
...
def _recent_parse_failures_check():
    ...
    return _check("recent_parser_errors", "Recent parser errors", status, f"{total} recent parser failure(s) found.", {"failed_items": failed_items, "failed_reports": failed_reports}, "Review parser errors. Full report bodies are intentionally omitted." if total else "")
...
def _recent_alert_generation_check():
    ...
    return _check("recent_alert_generation", "Recent alert generation", status, f"{alerts} alert(s) created from {events} recent event(s).", {"recent_events": events, "recent_alerts": alerts}, "Run alert rule evaluation and verify enabled rules." if status == "warning" else "")
...
def _defender_critical_ticket_check():
    ...
    return _check("defender_critical_auto_ticket", "Defender critical auto-ticket", "warning" if count else "ok", f"{count} Defender critical rule(s) lack auto ticket creation.", {"count": count}, "Enable auto ticket creation for Defender critical CVE rules." if count else "")
...
def _broad_critical_suppression_check():
    ...
    return _check("broad_critical_suppression", "Broad Critical suppression", "warning" if count else "ok", f"{count} broad Critical suppression rule(s) found.", {"count": count}, "Scope Critical suppressions narrowly and add expiration." if count else "")
...
def _critical_notification_check():
    ...
    return _check("critical_notification_channel", "Critical notification coverage", "ok" if enabled else "warning", f"{len(enabled)} enabled channel(s) can notify Critical alerts.", {"enabled_channels": [channel.name for channel in enabled]}, "Enable at least one notification channel for Critical alerts." if not enabled else "")
...
def _health_cards(checks):
    ...
    return [{"label": label, **by_code.get(code, _check(code, label, "warning", "Check not available."))} for label, code in mapping]
...
def _source_match_reason(source, sender="", subject="", body=""):
    ...
    if source.mailbox_sender_patterns and source.mailbox_subject_patterns:
        return (bool(sender_match and subject_match), f"sender pattern {sender_match}; subject pattern {subject_match}" if sender_match and subject_match else "sender and subject patterns did not both match")
    if sender_match:
        return True, f"sender pattern {sender_match}"
    if subject_match:
        return True, f"subject pattern {subject_match}"
    ...
    if token_match:
        return True, f"metadata token {token_match}"
    return False, "No sender, subject, or metadata pattern matched."
